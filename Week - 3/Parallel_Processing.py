        ...
        if (left is not None) and (right is not None):
            ...
            if left.time < right.time:
                ...
                if parent.time < left.time:
                    ...
                    if (parent.worker_id < left.worker_id) and (
                        parent.worker_id < right.worker_id
                    ):
                        break
                    elif (left.worker_id < parent.worker_id) and (
...
def run_tests():
    ...
    for test in tests:
        ...
        print()
        res = all([o[0] == t[0] and o[1] == t[1] for o, t in zip(output, test.output)])
        assert res, f"{test}\noutput: {output}"
