...
class Solver:
    ...
    def longest_common_substring(self):
        ...
        while a <= b:
            ...
            hashes_ss1_1 = self._precompute_substring_hashes(
                self.hashes_s1_1, len_, self._prime1
            )
            hashes_ss2_1 = self._precompute_substring_hashes(
                self.hashes_s2_1, len_, self._prime1
            )
            ...
            if same_hash1:
                ...
                hashes_ss1_2 = self._calc_substring_hash(
                    self.hashes_s1_2, self._prime2, i, len_
                )
                hashes_ss2_2 = self._calc_substring_hash(
                    self.hashes_s2_2, self._prime2, j, len_
                )
...
def run_stress_test():
    ...
    while True:
        ...
        assert (
            s1[answer.i : answer.i + answer.len] == s2[answer.j : answer.j + answer.len]
        ), f"""
				Input: s1={s1} | s2={s2}
				Got:      {answer}
				"""
