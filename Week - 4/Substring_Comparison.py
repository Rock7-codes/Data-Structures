...
class Solver:
    ...
    def check_equality(self, a, b, l):
        if l == 0:
            ...
            hash1_a = (
                self._calc_substring_hash(self.hashes1, self._prime1, a, l)
                % self._prime1
            )
            hash1_b = (
                self._calc_substring_hash(self.hashes1, self._prime1, b, l)
                % self._prime1
            )

            hash2_a = (
                self._calc_substring_hash(self.hashes2, self._prime2, a, l)
                % self._prime2
            )
            hash2_b = (
                self._calc_substring_hash(self.hashes2, self._prime2, b, l)
                % self._prime2
            )
